...
func (m *PodSandboxStatusResponse) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.AdditionalIps) > 0 {
		for iNdEx := len(m.AdditionalIps) - 1; iNdEx >= 0; iNdEx-- {
			i -= len(m.AdditionalIps[iNdEx])
			copy(dAtA[i:], m.AdditionalIps[iNdEx])
			i = encodeVarint(dAtA, i, uint64(len(m.AdditionalIps[iNdEx])))
			i--
			dAtA[i] = 0x12
		}
	}
...
func (m *PodSandboxStatusResponse) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 2:
			...
			m.AdditionalIps = append(m.AdditionalIps, string(dAtA[iNdEx:postIndex]))
			iNdEx = postIndex
		default:
			iNdEx = preIndex
			skippy, err := skip(dAtA[iNdEx:])
