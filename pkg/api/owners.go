...
//
// Note that OwningPlugins, FieldOwners and CompoundFieldOwners are not
// protected against concurrent access and therefore not goroutine safe.
//
...
func (f *FieldOwners) ClaimCompound(field int32, key, plugin string) error {
	if err := f.IsCompoundConflict(field, key, plugin); err != nil {
		return err
	}

	f.Compound[field].Owners[key] = plugin
	return nil
}
...
func (f Field) FieldName() string {
	name, ok := Field_name[int32(f)]
	if !ok {
		return fmt.Sprintf("<(unknown) field %d>", f)
	}
	return name
}
