...
const (
	// UnknownVersion is an alias of version.UnknownVersion.s
...
func InferVersionFromRuntime(runtime, runtimeVersion string) (string, error) {
	...
	if versionMap == nil {
		return unknown, fmt.Errorf("can't infer version for unknown runtime %q", runtime)
	}

	tag := version.FindClosestMatch(runtimeVersion, slices.Collect(maps.Keys(versionMap)))
	if tag == "" {
		return unknown, fmt.Errorf("failed to find latest version for %q", runtimeVersion)
	}

	nriVersion := versionMap[tag]
	if nriVersion == "" {
		return unknown, fmt.Errorf("unknown runtime version %q %q", runtime, runtimeVersion)
	}
