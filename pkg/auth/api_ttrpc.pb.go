...
func RegisterAuthenticationService(srv *ttrpc.Server, svc AuthenticationService) {
	srv.RegisterService("nri.pkg.auth.v1alpha1.Authentication", &ttrpc.ServiceDesc{
		Methods: map[string]ttrpc.Method{
			...
			"VerifyChallenge": func(ctx context.Context, unmarshal func(interface{}) error) (interface{}, error) {
				var req VerifyChallengeRequest
				if err := unmarshal(&req); err != nil {
					return nil, err
				}
				return svc.VerifyChallenge(ctx, &req)
			},
		},
	})
}
...
func (c *authenticationClient) VerifyChallenge(ctx context.Context, req *VerifyChallengeRequest) (*VerifyChallengeResponse, error) {
	var resp VerifyChallengeResponse
	if err := c.client.Call(ctx, "nri.pkg.auth.v1alpha1.Authentication", "VerifyChallenge", req, &resp); err != nil {
		return nil, err
	}
	return &resp, nil
}
