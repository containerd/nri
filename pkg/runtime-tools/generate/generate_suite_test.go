...
var _ = Describe("Adjustment", func() {
	When("nil", func() {
		It("does not modify the Spec", func() {
			...
			xg := xgen.SpecGenerator(rg, xgen.WithAuditLogger(al.Log))
	...
	When("has memory limit", func() {
		It("adjusts Spec correctly", func() {
			...
			Expect(al.Has(xgen.AuditSetLinuxResource,
				xgen.Fields{
					"type":  "memory limit",
					"value": "11111",
				})).To(BeTrue())
	...
	When("has a RDT adjustment", func() {
		It("adjusts Spec correctly", func() {
			...
			Expect(al.Has(xgen.AuditSetLinuxRdt,
				xgen.Fields{
					"closID": "foo",
				})).To(BeTrue())
			Expect(al.Has(xgen.AuditSetLinuxRdt,
				xgen.Fields{
					"schemata": "[L2:0=ff L3:0=f]",
				})).To(BeTrue())
			Expect(al.Has(xgen.AuditSetLinuxRdt,
				xgen.Fields{
					"monitoring": true,
