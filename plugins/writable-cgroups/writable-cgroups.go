...
const (
	...
	WritableCgroupsAnnotation = "writable-cgroups.noderesource.dev"
...
// isWritableCgroupsEnabled checks if the container has writable cgroups enabled.
func isWritableCgroupsEnabled(pod *api.PodSandbox, container *api.Container, l *logrus.Entry) bool {
	val, ok := nriplugin.GetEffectiveAnnotation(pod, WritableCgroupsAnnotation, container.Name)
	if ok {
		l.WithFields(logrus.Fields{
			"annotation": WritableCgroupsAnnotation,
			"value":      val,
		}).Debug("Found effective annotation")
		return val == "true"
	}

	l.WithField("annotation", WritableCgroupsAnnotation).Debug("Annotation not found")
	return false
}
